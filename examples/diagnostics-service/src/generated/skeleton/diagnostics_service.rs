...
use std::sync::Arc;
pub struct DiagnosticsServiceSkeleton<T: Transport> {
    pub base: SkeletonBase<T>,
}
impl<T: Transport> DiagnosticsServiceSkeleton<T> {
    pub fn new(transport: Arc<T>, instance_id: InstanceId) -> Self {
        Self {
            base: SkeletonBase::new(transport, ServiceId(20480), instance_id),
        }
    }
    ...
    pub async fn notify_dtc_status_changed(
        &self,
